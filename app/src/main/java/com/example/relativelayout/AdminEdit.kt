...
class AdminEdit : AppCompatActivity(){
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val UpdatePost= findViewById<Button>(R.id.button4)
        ...
        UpdatePost.setOnClickListener {
            if (postIdToFetch != null) {
                updatePostInDatabase(postIdToFetch)
                // Get the DatabaseReference for the specific post
                //val postReference = postsReference.child(postIdToFetch)
                val intent=Intent(this,AdminView::class.java)
                startActivity(intent)
    ...
    private fun PostDetails(container:LinearLayout,name:String?,content:String?,detailedDescription: String?,venue:String?,date:String?) {
        ...
        addEditText(container, "Content: ", content,"content")
        addEditText(container, "Description: ", detailedDescription,"detailedDescription")
        addEditText(container, "Venue: ", venue,"venue")
        addEditText(container, "Date: ", date,"date")
    }
    ...
    private fun addEditText(container: LinearLayout, labelText: String, respectiveContent: String?,fieldName: String) {
        val linearLayout = LinearLayout(this)
        linearLayout.layoutParams = LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
            LinearLayout.LayoutParams.WRAP_CONTENT
        )
        linearLayout.orientation = LinearLayout.HORIZONTAL // Set orientation to horizontal

        // Add TextView for label and respective content
        val labelTextView = TextView(this)
        labelTextView.text = labelText
        labelTextView.textSize = 18f
        labelTextView.setTextColor(ContextCompat.getColor(this, R.color.black))
        linearLayout.addView(labelTextView)

        // Add EditText for respective content
        val editText = EditText(this)
        editText.layoutParams = LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
            LinearLayout.LayoutParams.WRAP_CONTENT,
            1f // Set weight to make EditText fill remaining space
        )
        editText.textSize = 18f
        editText.setText(respectiveContent)
        ...
        // Save the EditText reference to a map with the field name as the key
        editTextMap[fieldName] = editText
    }
    private fun updatePostInDatabase(postId: String) {
        // Get reference to the Firebase database
        val database = FirebaseDatabase.getInstance()
        val postsRef = database.getReference("posts")

        // Construct the update map with the new values from EditText fields
        val updateMap = HashMap<String, Any>()
        for ((fieldName, editText) in editTextMap) {
            updateMap[fieldName] = editText.text.toString()
        }

        // Update the post data in the database
        postsRef.child(postId).updateChildren(updateMap)
            .addOnSuccessListener {
                // Update successful
                //println("Post updated successfully in the database.")
                Toast.makeText(baseContext,"Post updated successfully in the database.",Toast.LENGTH_LONG).show()
            }
            .addOnFailureListener { exception ->
                // Update failed
                Toast.makeText(baseContext,"Error updating post.",Toast.LENGTH_LONG).show()
                //println("Error updating post: $exception")
            }
    }
